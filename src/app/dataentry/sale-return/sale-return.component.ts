...
export class SaleReturnComponent implements OnInit, OnDestroy {
  ...
  async fetchBrokersAndAreas(): Promise<void> {
    await Promise.all([this.fetchBrokers(), this.fetchAreas(), this.fetchCategories(2), this.fetchSuppliers(), this.fetchItems(), this.fetchUnits(), this.fetchFields(), this.fetchGroupMapping(), this.fetchSaleReturnAccounts(), this.fetchDynamicFields()]);
  }
  ...
  fetchSummary(): void {
    ...
    this.entryService.getTaxSummary(userId, this.financialYear, 4, fromDateStr || undefined, toDateStr || undefined).subscribe(data => {
  ...
  fetchEntries(): void {
    ...
    const fromDateStr = this.datePipe.transform(this.fromDate, 'yyyy-MM-dd', 'en-IN') as string; // Transform to desired format
    const toDateStr = this.datePipe.transform(this.toDate, 'yyyy-MM-dd', 'en-IN') as string; // Transform to desired format  
    this.entryService.getEntriesByUserIdAndFinancialYearAndType(userId, this.financialYear, 4, this.nextStartRow, this.pageSize, fromDateStr || undefined, toDateStr || undefined).subscribe(data => {
  ...
  fetchSaleReturnAccounts(): Promise<void> {
    return new Promise((resolve) => {
      const userId = this.storageService.getUser().id;
      this.accountService.getAccountsByUserIdAndFinancialYear(userId, this.financialYear, ['Sale Return Account']).subscribe((accounts: any[]) => {
        accounts.forEach(account => {
  ...
  openEditGroupDialog(group: any, type: number = 4): void {
    const dialogRef = this.dialog.open(AddEditEntryDialogComponent, {
      width: '1250px',
  ...
  deleteGroup(invoice_seq_id: number): void {
    this.entryService.deleteEntries(invoice_seq_id, 4).subscribe((result) => {
      if (result) {
  ...
  subscribeToWebSocketEvents(): void {
    ...
    const handleEvent = (data: any, action: 'INSERT' | 'UPDATE' | 'DELETE') => {
      ...
      if (data.entryType === 'entry' && data.data.group.type === 4 && data.user_id === currentUserId && data.financial_year === currentFinancialYear &&
