...
export class CreditNoteComponent implements OnInit, OnDestroy {
  ...
  fetchSummary(): void {
    ...
    this.entryService.getTaxSummary(userId, this.financialYear, 5, fromDateStr || undefined, toDateStr || undefined).subscribe(data => {
  ...
  fetchEntries(): void {
    ...
    const fromDateStr = this.datePipe.transform(this.fromDate, 'yyyy-MM-dd', 'en-IN') as string; // Transform to desired format
    const toDateStr = this.datePipe.transform(this.toDate, 'yyyy-MM-dd', 'en-IN') as string; // Transform to desired format
    this.entryService.getEntriesByUserIdAndFinancialYearAndType(userId, this.financialYear, 5, this.nextStartRow, this.pageSize, fromDateStr || undefined, toDateStr || undefined).subscribe(data => {
  ...
  deleteGroup(invoice_seq_id: number): void {
    this.entryService.deleteEntries(invoice_seq_id, 5).subscribe((result) => {
      if (result) {
