...
export class DebitNoteComponent implements OnInit, OnDestroy {
  ...
  async fetchBrokersAndAreas(): Promise<void> {
    await Promise.all([this.fetchBrokers(), this.fetchAreas(), this.fetchCategories(2), this.fetchSuppliers(), this.fetchItems(), this.fetchUnits(), this.fetchFields(), this.fetchGroupMapping(), this.fetchDebitNoteAccounts(), this.fetchDynamicFields()]);
  }
  ...
  fetchSummary(): void {
    ...
    this.entryService.getTaxSummary(userId, this.financialYear, 6, fromDateStr || undefined, toDateStr || undefined).subscribe(data => {
  ...
  fetchEntries(): void {
    ...
    const fromDateStr = this.datePipe.transform(this.fromDate, 'yyyy-MM-dd', 'en-IN') as string; // Transform to desired format
    const toDateStr = this.datePipe.transform(this.toDate, 'yyyy-MM-dd', 'en-IN') as string; // Transform to desired format  
    this.entryService.getEntriesByUserIdAndFinancialYearAndType(userId, this.financialYear, 6, this.nextStartRow, this.pageSize, fromDateStr || undefined, toDateStr || undefined).subscribe(data => {
  ...
  fetchDebitNoteAccounts(): Promise<void> {
    return new Promise((resolve) => {
      const userId = this.storageService.getUser().id;
      this.accountService.getAccountsByUserIdAndFinancialYear(userId, this.financialYear, ['Debit Note Account']).subscribe((accounts: any[]) => {
        accounts.forEach(account => {
  ...
  openEditGroupDialog(group: any, type: number = 6): void {
    const dialogRef = this.dialog.open(AddEditEntryDialogComponent, {
      width: '1250px',
  ...
  deleteGroup(invoice_seq_id: number): void {
    this.entryService.deleteEntries(invoice_seq_id, 6).subscribe((result) => {
      if (result) {
  ...
  subscribeToWebSocketEvents(): void {
    ...
    const handleEvent = (data: any, action: 'INSERT' | 'UPDATE' | 'DELETE') => {
      ...
      if (data.entryType === 'entry' && data.data.group.type === 6 && data.user_id === currentUserId && data.financial_year === currentFinancialYear &&
