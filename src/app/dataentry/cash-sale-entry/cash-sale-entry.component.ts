...
@Component({
  ...
  templateUrl: './cash-sale-entry.component.html',
  styleUrl: './cash-sale-entry.component.css'
})
export class CashSaleEntryComponent implements OnInit, OnDestroy {
  ...
  fetchSummary(): void {
    ...
    this.entryService.getTaxSummary(userId, this.financialYear, 8, fromDateStr || undefined, toDateStr || undefined).subscribe(data => {
  ...
  fetchEntries(): void {
    ...
    const fromDateStr = this.datePipe.transform(this.fromDate, 'yyyy-MM-dd', 'en-IN') as string; // Transform to desired format
    const toDateStr = this.datePipe.transform(this.toDate, 'yyyy-MM-dd', 'en-IN') as string; // Transform to desired format
    this.entryService.getEntriesByUserIdAndFinancialYearAndType(userId, this.financialYear, 8, this.nextStartRow, this.pageSize, fromDateStr || undefined, toDateStr || undefined).subscribe(data => {
  ...
  updateEntriesWithDynamicFields(data: any[]): any[] {
    data.forEach(entry => {
      ...
      entry.account_name = this.accountMap[entry.account_id] || 'NA';
      ...
      entry.gstNo = this.gstNoMap[entry.account_id] || 'NA';
  ...
  openAddEntryDialog(): void {
    const dialogRef = this.dialog.open(AddEditEntryDialogComponent, {
      width: '1250px',
      data: { userId: this.storageService.getUser().id, financialYear: this.financialYear, type: 8 }
    });
  ...
  openEditGroupDialog(group: any, type: number = 8): void {
    const dialogRef = this.dialog.open(AddEditEntryDialogComponent, {
      width: '1250px',
      data: { group, userId: this.storageService.getUser().id, financialYear: this.financialYear, type }
    });
  ...
  deleteGroup(invoice_seq_id: number): void {
    this.entryService.deleteCashEntries(invoice_seq_id, 8).subscribe((result) => {
      if (result) {
  ...
  subscribeToWebSocketEvents(): void {
    ...
    const handleEvent = (data: any, action: 'INSERT' | 'UPDATE' | 'DELETE') => {
      ...
      const entryDate = new Date(data.entry_date).getTime(); // Convert entry_date to timestamp for comparison
      ...
      if (data.entryType === 'entry' && data.data.group.type === 8 && data.user_id === currentUserId && data.financial_year === currentFinancialYear &&
  ...
  updateCache(page: EntryCachedPage, action: 'INSERT' | 'UPDATE' | 'DELETE', group: any): void {
    switch (action) {
      case 'INSERT':
  ...
  handleInsert(data: any): void {
    ...
    data.data.entries.forEach((entry: any) => {
      ...
      if (!group) {
        group = {
          ...
          entry_date: entry.entry_date,
          sNo: entry.sNo,
          customerName: this.accountMap[entry.account_id] || 'NA',
          gstNo: this.gstNoMap[entry.account_id] || 'NA',
          account_id: entry.account_id,
          groupEntryValue: 0,
          groupTotalAmount: 0,
          entries: []
    ...
    for (const [pageNumber, page] of this.cache.entries()) {
      ...
      if (
        ...
        this.updateCache(page, 'INSERT', group);
        ...
        if (!(new Date(group.entry_date).getTime() >= page.dataRange.start &&
          new Date(group.entry_date).getTime() <= page.dataRange.end)) {
  ...
  handleUpdate(data: any): void {
    ...
    for (const [pageNumber, page] of this.cache.entries()) {
      if ((new Date(group.entry_date).getTime() >= page.dataRange.start
        ...
        this.updateCache(page, 'UPDATE', group);
        if (Number(pageNumber) === this.currentPage) {  // Convert pageNumber to a number before comparison
          this.groupedEntries = page.data;
        }
        if (!(new Date(group.entry_date).getTime() >= page.dataRange.start &&
  ...
  handleDelete(data: any): void {
    ...
    const entry_date = data.data.group.entry_date;
    ...
    for (const [pageNumber, page] of this.cache.entries()) {
      if (new Date(entry_date).getTime() >= page.dataRange.start
        ...
        console.log(pageNumber);
        console.log(page);
        this.updateCache(page, 'DELETE', { invoice_seq_id: invoice_seq_id });
        if (Number(pageNumber) === this.currentPage) {  // Convert pageNumber to a number before comparison
          this.groupedEntries = page.data;
        }
        return;
